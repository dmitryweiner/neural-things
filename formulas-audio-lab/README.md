...
- **Start Audio** — запуск AudioContext (выделена зелёным цветом для заметности)
...
- **Scope** — показать/скрыть осциллоскоп (когда скрыт — панель полностью убирается)
...
1. Управление воспроизведением: Start Audio, Stop, Record
...
- Сворачивается кнопкой "Scope" в верхней панели
...
- **Свернуть все** — сворачивает все формулы для компактного вида
...
**Для пользователей:** Нажмите кнопку "Start Audio" — звук активируется автоматически.
...
- При нажатии "Start Audio" запрашивается wake lock — экран не будет гаснуть
- При нажатии "Stop" wake lock освобождается
