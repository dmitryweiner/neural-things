...
class Game {
    ...
    update(deltaTime) {
        ...
        if (this.train.speed < TRAIN_MAX_SPEED) {
            this.train.speed = Math.min(TRAIN_MAX_SPEED, this.train.speed + TRAIN_ACCELERATION * deltaTime);
        }
        ...
        if (turnCell) {
            const [nextX, nextY, nextDirection] = turnCell(this.train.pixelX, this.train.pixelY, this.train.speed, this.train.direction, deltaTime);
            nextPixelX = nextX;
            nextPixelY = nextY;
            ...
            switch (this.train.direction) {
                case DIRECTIONS.right: nextPixelX += this.train.speed * CELL_SIZE * deltaTime; break;
                case DIRECTIONS.left: nextPixelX -= this.train.speed * CELL_SIZE * deltaTime; break;
                case DIRECTIONS.up: nextPixelY -= this.train.speed * CELL_SIZE * deltaTime; break;
                case DIRECTIONS.down: nextPixelY += this.train.speed * CELL_SIZE * deltaTime; break;
            }
        }
        ...
        if (nextGridX !== this.train.x || nextGridY !== this.train.y) {
            ...
            if (this.isValidMove(nextGridX, nextGridY)) {
                ...
                if (TURN_DIRECTIONS[cellType] && TURN_DIRECTIONS[cellType][this.train.direction]) {
                    this.train.direction = TURN_DIRECTIONS[cellType][this.train.direction];
                }
            } else {
